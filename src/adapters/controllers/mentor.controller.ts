...
import { MentorPresenter } from "../presenters/mentor.presenter";

export class MentorController {

    private mentorUseCase: IMentorUseCase
    ...
    //         const user = await this.mentorUseCase.signup({ name , email, phone, password, role: "mentee"});
    //         console.log(user, "this is the user in the controller ...")
    //         res.status(201).json(MentorPresenter.SignUpRes(true, "User created and OTP sent successfully", user.email));
    //     }
    //     catch(error) {
    //         const errorMessage = error instanceof Error ? error.message : 'Unknown error';
    //         const errorCode = (error as any).code || 500; // Default to 500 if no code is provided
    //         res.status(errorCode).json(MentorPresenter.ErrorRes({ message: errorMessage, code: errorCode }));
    //     }
    // }
    ...
    //         res.cookie("mentorAccessToken", token, {
    //           httpOnly: true,
    //           maxAge: 360000,
    //         }).cookie("mentorRefreshToken", refreshToken, {
    //           httpOnly: true,
    ...
    //     try {
    //       res.cookie("mentorAccessToken", "", { httpOnly: true, expires: new Date() }).cookie("mentorRefreshToken", "", { httpOnly: true, expires: new Date() })
    ...
    //     const response = await this.mentorUseCase.updateUser(email , profilePic, coverPic)
    //     res.status(200).json(response)
    //     } catch (error) {
    //     console.log(error);
    //     }
    ...
    //         const response = await this.mentorUseCase.editDetails(name,phone,email,education,bio);
    ...
    //       httpOnly: true,
    //       maxAge: 360000,
    //     }).cookie("mentorRefesehToken", refreshToken, {
    //       httpOnly: true,
    //       maxAge: 30 * 24 * 60 * 60 * 1000
    //     })
